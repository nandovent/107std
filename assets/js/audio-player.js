(()=>{
  ...
  const update=()=>{
    ...
    status.textContent=audio.paused?'PAUSADO':audio.muted?'TOCANDO · MUDO':'TOCANDO';
  };
  const start=async()=>{
    audio.volume=.72;
    try{
      ...
      try{audio.muted=true;await audio.play();}catch(ignore){status.textContent='APERTE O PLAY';}
    }
    update();
  };
