(() => {
  ...
  const activate=(carousel)=>{
    ...
    const buildGrid=()=>{grid.innerHTML='';slides.forEach((s,i)=>{const b=document.createElement('button');b.className='portfolio-grid-card';b.type='button';b.innerHTML=`<div class="portfolio-grid-thumb"><img src="${esc(s.dataset.thumb)}" alt="Miniatura de ${esc(s.dataset.title)}"><span class="portfolio-grid-play">▶</span></div><div class="portfolio-grid-copy"><strong>${esc(s.dataset.title)}</strong><span>${esc(s.dataset.label)} · ${esc(s.dataset.source)}</span></div>`;b.onclick=()=>{go(i);modal.classList.remove('is-open');document.body.classList.remove('modal-open');carousel.scrollIntoView({behavior:'smooth',block:'center'})};grid.appendChild(b)})};
  ...
  fetch('../data/portfolio.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error();return r.json()}).then(items=>{
    ...
    root.innerHTML=Object.entries(groups).map(([cat,list])=>`<section class="panel light reveal visible"><div class="category-heading"><div><span class="kicker">Categoria</span><h2 class="title">${esc(cat)}.</h2></div></div><div class="portfolio-list-shell" data-list-carousel><div class="portfolio-list-actions"><div class="portfolio-arrows"><button class="portfolio-icon-button" data-list-prev>←</button><button class="portfolio-icon-button" data-list-next>→</button></div><button class="portfolio-expand-button" data-list-expand>Expandir</button></div><div class="portfolio-list-viewport"><div class="portfolio-list-track" data-list-track>${list.map(v=>`<article class="portfolio-list-slide" data-list-slide data-title="${esc(v.titulo)}" data-label="${esc(cat)}" data-source="${esc(v.video.platform)}" data-thumb="${esc(v.video.thumb)}"><div class="video-frame"><iframe src="${esc(v.video.embed)}" title="${esc(v.titulo)}" allow="autoplay; fullscreen; picture-in-picture" allowfullscreen loading="lazy"></iframe></div><div class="portfolio-info portfolio-info-below"><span class="kicker">${esc(cat)}</span><h3>${esc(v.titulo)}.</h3><p>${esc(v.descricao)}</p></div></article>`).join('')}</div></div></div><div class="portfolio-grid-modal" data-list-grid><div class="portfolio-grid-backdrop" data-list-close></div><div class="portfolio-grid-dialog"><div class="portfolio-grid-header"><div><span class="kicker">Todos os vídeos</span><h3>${esc(cat)}.</h3></div><button class="portfolio-icon-button" data-list-close>×</button></div><div class="portfolio-grid" data-generated-grid></div></div></div></section>`).join('')||'<div class="portfolio-empty">Nenhum vídeo publicado.</div>';
