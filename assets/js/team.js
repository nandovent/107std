(()=>{
  ...
  const render=items=>{
    ...
    grid.innerHTML=visible.map((item,index)=>{
      ...
      const profileLink=profile?`<a class="home-team-instagram" href="${esc(profile.url)}" target="_blank" rel="noopener" aria-label="Instagram de ${esc(item.nome)}">${esc(profile.handle)}</a>`:'';
      return `<article class="home-team-member${featured?' home-team-member-featured':''}"><div class="home-team-photo${item.arquivo?' has-image':''}" aria-label="Foto de ${esc(item.nome)}">${image}</div><div class="home-team-info"><h3>${esc(item.nome)}</h3><p>${esc(item.funcao)}</p>${profileLink}</div></article>`;
    }).join('');
  };
  const init=async()=>{
    try{
