...
async function putContent(path,content,message,sha){
  ...
  const payload={message,content:Buffer.isBuffer(content)?content.toString('base64'):Buffer.from(content).toString('base64'),branch:BRANCH};
...
})[String(path).split('.').pop().toLowerCase()]||'application/octet-stream';
...
export default async function handler(req,res){
  ...
  try{
    // Rota pública usada pelo carrossel e pelas miniaturas do painel.
    const asset=typeof req.query?.asset==='string'?req.query.asset:'';
    if(req.method==='GET'&&asset){
      const safe=asset.replace(/^\/+/, '');
      if(!/^assets\/clientes\/[a-zA-Z0-9_-]+\.(png|jpe?g|webp|svg)$/i.test(safe)){
    ...
    for(const raw of body.items){
      ...
      if(item.upload&&item.upload.dataUrl){
        ...
        await putContent(path,Buffer.from(match[2],'base64'),`Atualiza logo de cliente: ${item.nome||safe}`,existing?.sha);
        item.arquivo=`/${path}`;
      }
      delete item.upload;
      next.push(item);
